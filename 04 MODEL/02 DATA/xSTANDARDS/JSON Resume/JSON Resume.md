# JSON Resume (JR) Ecosystem

* [JSON Resume (JR)](../../../../GLOSS/J/JSON%20Resume.md) in the **Glossar**
...
JSON Resume, along the project shortly referred to as "**JR**",  is an older, but widely accepted open-source **JSON Schema** for resumees that comes with a set of 3rd-party tools to render such CV-data with hundreds of community themes, host it for free, and export it to HTML, PDF, and more.

**The gold** lies in its widely accepted and supported **ECO-SYSTEM of many Resume Editors, data-validation and format-conversion tools, website-builders, hosting solutions, etc.** 

However, the core project seems to have been abondend in 2019, whereas its commuity and apps relying and supporting this standard are still alive, including the following projects:

* [Reactive Resume](../Reactive%20Resume/_Reactive%20Resume.md) a professional looking Wysiwyg Resume Editor with [JSON Resume](JSON%20Resume.md) export we are going to use along the ApplAi project for manually colleting raw CV data in JSON format to split it in a next step into the atomic YAML formatted CV atoms.  
